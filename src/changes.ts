import { textChanges, SourceFile, Node, TextRange, startEndOverlapsWithStartEnd, BaseChange } from "typescript";
...
export class ProxyChangesTracker implements textChanges.ChangeTracker {
    ...
    replaceNode(sourceFile: SourceFile, oldNode: Node, newNode: Node, options?: textChanges.ChangeNodeOptions): void {
        ...
        if (lastChange) {
            ...
            if (changes.some(c => startEndOverlapsWithStartEnd(c.range.pos, c.range.end, lastChange.range.pos, lastChange.range.end))) {
                this.popLastChanges()
